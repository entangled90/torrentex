defmodule Torrentex.Torrent.Reporter do
  ...
  defstruct [:files_writer, :num_pieces, :piece_length, pieces_downloaded: 0, active_peers: 0, downloading_pieces: 0]
  ...
  def init(args) do
    ...
    {:ok, %__MODULE__{files_writer: files_writer, num_pieces: num_pieces, piece_length: piece_length}}
  end
  ...
  def handle_info(:report, %__MODULE__{} = state) do
    ...
    Logger.info "Downloaded pieces: #{size}/#{state.num_pieces} = #{size / state.num_pieces}. Speed #{speed}MB/s"
    Process.send_after(self(), :report, @report_interval)
